...
make stress-clean                # tear down the stress environment and remove its volumes + images
...
`grafana/k6:2.1.0` image.
The stress harness runs inside an **isolated Docker project** (`flash-sale-stress`)
so the dev stack keeps running throughout — its Redis, MongoDB, and API are
never touched. The stress environment publishes its stores on loopback-only
alternative ports (Redis on `6380`, MongoDB on `27018`, API on `3001`). The
harness stops the stress API, resets the stress stores, restarts the stress API,
drives the concurrent burst with k6, then verifies the results. Redis
(`SCARD orders:{saleId}:users` + `stock:{saleId}:remaining`) is the authoritative
fairness record: every fairness count is an exact equality against the API's own
seeded stock. The async Mongo audit is reconciled with a tolerance — an accepted
under-count passes with a note, while an over-count hard-fails. It then
re-checks that a past-window sale rejects every attempt.
...
When you are done, clean up the stress environment with:
```bash
make stress-clean                # docker compose down -v --rmi local for the stress project
```
### Expected outcome
...
<!-- stress:latest -->
**Latest stress report:** [20260716_0348](docs/testing/stress/20260716_0348/report.html)
<!-- /stress:latest -->
### Stress configuration
The stress harness uses `.env.stress` — a separate Docker project (`flash-sale-stress`)
with its own Redis, MongoDB, API, and worker ensures a stress run never touches
the dev stack. The harness and the API container always agree on `STOCK_QUANTITY`
via this file. The sale window is seeded dynamically by `seed-db.ts
...
The stress stores are published on alternative loopback ports so both environments
coexist without conflict:
| Service | Dev port | Stress port |
| --- | --- | --- |
| Redis | 6379 | 6380 |
| MongoDB | 27017 | 27018 |
| API | 3000 | 3001 |
...
docker-compose.yml      dev/production stack: api · worker (profile) · client (nginx) · redis:8-alpine · mongo:8
docker-compose.stress.yml  self-contained stress environment (project flash-sale-stress): own redis (6380) ·
                           mongo (27018) · api (3001) · worker — never shares ports or volumes with the dev stack
Makefile                install / seed / dev / build / deploy / stress / stress-clean / clean / worker-logs targets
```
## Domain model
